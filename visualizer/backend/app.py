...
def check_dataset(path: str) -> CheckDatasetResponse:
    ...
    """
    if not DatasetInferenceStore.db_exists(path):
        return CheckDatasetResponse(has_db=False)

    store = DatasetInferenceStore(path)
...
def create_job(request: JobRequest) -> JobStatusResponse:
    ...
    db_already_exists = DatasetInferenceStore.db_exists(request.dataset_path)
...
def cancel_semantic_search(job_id: str, search_id: str) -> dict:
    ...
    if search_job.status in ["done", "error", "cancelled"]:
        raise HTTPException(400, f"Cannot cancel. Search job is already in status: '{search_job.status}'")
...
def _store_can_resume(store: DatasetInferenceStore, status: str | None) -> bool:
